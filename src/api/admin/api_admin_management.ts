...
export const api = {
          ...
          updateProduct: async (
            prodName: string, valueResale: number, cust: number, tax: number = 0, supplier: string = '', discount: number = 0, description: string = '', qt: number = 0, datePurchase: Date | string, sector: string = '', 
            supplierCode: string = '', manufacturer: string = '', weight: number = 0, weightUnit: string = '', dimensions: string = '', dimensionsUnit: string = '', barCode: string = '', lastUpdate:  Date | string | null = null, 
            reorderPoint: number = 0, restockTime: number = 0, warrantyInfo: string = '', batchInfo: string = '', expireDate: Date | string | null = null, materialOrIngredients: string = '', safetyRating: string = '', shippingRestrictions: string = '', producToken: string = ''
          ): Promise<ResponseData> => {
            try {
              const token = localStorage.getItem('token') || '';
              const refreshToken = localStorage.getItem('refreshToken') || '';
              ...
              let response = await fetch(`http://127.0.0.1:5000/axiosadmin/products/products_update/${producToken}`, {
                method: 'PUT',
                body: JSON.stringify({
                  prodName: prodName,
                  valueResale: valueResale,
